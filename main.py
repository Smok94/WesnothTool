...
class WMLGenerator():
    def __init__(self):
        self.header = "###Generated by Wesnoth Addon Generator "+VERSION+"###\n"

    def genUnit(self, unit):
        f = open(PATH_ADDON+"/units/"+getattr(unit, "id")+".cfg", "w")
        f.write("[unit_type]\n")
        atts = [["experience", 0], ["hitpoints", 0], ["id", 0], ["image", 1], ["movement", 0], ["name", 2]]
        for a in atts:
            if a[1] == 0:
                ln = "    "+a[0]+" = "+getattr(unit, a[0])+"\n"
            elif a[1] == 1:
                f.write("    "+a[0]+" = \""+getattr(unit, a[0])+"\"\n")
            elif a[1] == 2:
                f.write("    "+a[0]+" = _ \""+getattr(unit, a[0])+"\"\n")
            f.write(ln)
        f.write("[/unit_type]\n")
        f.close()
